...
abstract class Buffer
{
     ...
     * @param int       $char     The Unicode character code to add
     *
     * @return int                The glyphId for $char in the subset font
     *
     * @throws FontException
     */
    public function addSubsetChar(string $key, int $char): int
    {
        if (!isset($this->font[$key])) {
            throw new FontException("The font $key must be loaded before adding subset characters.");
        }
        $this->font[$key]['subsetchars'][$char] ??= 1 + \count($this->font[$key]['subsetchars']);

        return $this->font[$key]['subsetchars'][$char];
    }
     ...
     *                       order to make changes to your PDF. The file size of the PDF would also be smaller
     *                       because you are embedding only a subset. Set this to null to use the default value.
